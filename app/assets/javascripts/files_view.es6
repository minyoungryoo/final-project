...
// 16-18, 38-49
		...
		for(var i=0; i<=14; i++){
			if (i<=2){
			content += `<input type="checkbox" data-patient-id=${patient_id} class="searchType" name=${content_label[i]} data-index=${i} data-id=${i+16}>${content_label[i]}`;
			}else{
			content += `<input type="checkbox" data-patient-id=${patient_id} class="searchType" name=${content_label[i]} data-index=${i} data-id=${i+35}>${content_label[i]}`;
			};
		};
